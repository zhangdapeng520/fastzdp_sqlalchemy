...
class Employee(BaseModel):
    ...
    department_id: Mapped[int] = mapped_column(sqlalchemy.ForeignKey("department.id"), comment="部门ID")
...
if __name__ == '__main__':
    with sessionmaker(engine).begin() as session:
        ...
        zs = Employee(name="张三", age=23, salary=3000, bonus=0, gender=GenderEnum.MALE.value)
        ls = Employee(name="李四", age=34, salary=21000, bonus=10000, gender=GenderEnum.MALE.value)

        department.employees = [zs, ls]
