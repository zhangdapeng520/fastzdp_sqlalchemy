...
if __name__ == '__main__':
    with sessionmaker(engine).begin() as session:
        card_id = "188888888888888888"

        # 查询card_id对应的员工姓名及其部门名
        # isourter=True 表示外连接，如果员工没有部门，也能被查询出来
        query = select(IdCard.card_id, Employee.name, Department.name).join(Department, isouter=True).join(
            IdCard).where(IdCard.card_id == card_id)
