...
if __name__ == '__main__':
    with sessionmaker(engine).begin() as session:
        # 查询角色数量大于1的用户
        child_query = select(User.username, func.count(Role.id).label("role_count")).join(User.roles).group_by(User.id)
        query = select(child_query.c.username, child_query.c.role_count).where(child_query.c.role_count > 1)
